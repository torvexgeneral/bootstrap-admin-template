...
async function startAstroServer(options = {}) {
  ...
  return new Promise((resolve, reject) => {
    ...
    const startTimeout = setTimeout(() => {
      if (!serverStarted) {
        reject(
          new Error("Astro server failed to start within the timeout period")
        )
      }
    ...
    setTimeout(async () => {
      try {
        ...
        log(`Also available at http://${host === "0.0.0.0" ? "your-local-ip" : host}:${port}${startPath}`, "success")
  ...
  try {
    ...
    if (watchCleanup) {
      try {
        ...
        log(
          `Error cleaning up file watchers: ${cleanupError.message}`,
          "error"
        )
      }
    }
